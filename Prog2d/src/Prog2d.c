...
int main(void) {
	...
	int *totalHistogram = malloc(NUMBINS * sizeof(int)*2);
	...
	for(int cnt = 0; cnt < NUMBINS; cnt++ ) {
		if ( totalHistogram[cnt] > maxVal ) maxVal = totalHistogram[cnt];
		if ( totalHistogram[cnt] < minVal ) minVal = totalHistogram[cnt];
	}
